...
class TransformerBlockInitComputationTP(Operator):
    ...
    def __call__(self, x: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        K_cache.set_desc("K_cache_")

        V_cache = Tensor([b, h, s, d_h], self.data_type)
        V_cache.set_desc("V_cache_")
        ...
        for device_id in range(device_count):
            base_device_id = device_id * self.device_count_sz
            ...
            _ = AllChipletBarrierSync(self.data_type)()
            
            # maps a core to the most recent tensor it produces
            most_recent_tensor = {}
            all_out_tensors = []
            for i in range(start_head, end_head):
                # target_core_id = min((device_id * self.device_count_sz) + (i%self.device_count_sz), (device_id + 1) * self.device_count_sz - 1)
                target_core_id = min((device_id * self.device_count_sz) + (self.device_count_sz - 1), (device_id + 1) * self.device_count_sz - 1)
                ...
                q_mul_k_single_matmul = Matmul(self.V_transpose[device_id].data_type)
...
class TransformerBlockAutoRegressionTP(Operator):
    def __init__(self, d_model, n_heads, d_head, device_count, data_type: DataType, device_count_sz: int = 8, prefill_init_obj: Optional[TransformerBlockInitComputationTP] = None):
        super().__init__(0, 0, 0, 0, data_type)
        self.d_model = d_model
        self.n_heads = n_heads
        self.d_head = d_head
        ...
        d = d_model
        if prefill_init_obj:
            self.Wq = prefill_init_obj.Wq
            self.Wk = prefill_init_obj.Wk
            self.Wv = prefill_init_obj.Wv
            
            self.W0 = prefill_init_obj.W0
            self.W1 = prefill_init_obj.W1
            self.W2 = prefill_init_obj.W2
        else:
            self.Wq = Tensor([d, d], data_type)
            self.Wq.set_desc("Wq")
...
class GPTModel:
    def __init__(self, d_model:int, n_heads:int, d_head:int, n_blocks:int, device_count:int, data_type, device_count_sz):
        ...
        self.blocks_decode = [
            TransformerBlockAutoRegressionTP(d_model, n_heads, d_head, device_count, data_type, device_count_sz, self.blocks_init[block_i])
...
if __name__ == "__main__":
    ...
    if prefill_active:
        ...
        dep_graph_path = Path("prefill_gpt3_small_128_tokens.json")
    ...
    dep_graph_path = Path("prefill_decode_gpt3_small_128_tokens.json")
